...
export async function PATCH(request: Request, { params }: { params: { id: string; imagenId: string } }) {
  try {
    ...
    const supabase = createSupabaseServerClient();
    const { error } = await supabase.from('producto_imagenes').update(parsed.data).eq('id', params.imagenId);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
...
export async function DELETE(_request: Request, { params }: { params: { id: string; imagenId: string } }) {
  try {
    ...
    const admin = createSupabaseAdminClient();
    const { error } = await admin.from('producto_imagenes').update({ activo: false }).eq('id', params.imagenId);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
