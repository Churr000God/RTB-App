...
export async function PATCH(request: Request, { params }: { params: { tipo: string; id: string } }) {
  try {
    ...
    // B-01 (contexto/QA_INTEGRAL_2026-08-06.md): sin .select(), un registro
    // fuera de RLS (rol sin permiso sobre este catálogo) devolvía 200 sin
    // editarse.
    const supabase = createSupabaseServerClient();
    const { data, error } = await supabase.from(meta.tabla).update(parsed.data).eq('id', params.id).select('id');
